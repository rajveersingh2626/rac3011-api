...
export function publicProjectSummaryDto(row: PublishedProjectRow) {
  ...
  return {
    ...
    category: row.category || row.areasOfFocus?.[0] || row.avenueOfService || '',
    avenueOfService: row.avenueOfService ?? null,
    areasOfFocus: (row.areasOfFocus && row.areasOfFocus.length > 0) ? row.areasOfFocus : (row.category ? [row.category] : []),
