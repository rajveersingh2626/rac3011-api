...
function ctxWith(grants: Record<string, { type: string }[]>): RequestContext {
  return {
    ...
    sessionId: 's1',
    access: { userId: 'user-1', isSuperAdmin: false, roles: [], grants } as never,
  };
}
...
function fakeRepo(initial?: ContentBlockAdminRow) {
  ...
  return {
    ...
    findOne: vi.fn(() => row),
    upsertDraft: vi.fn((pageKey: string, sectionKey: string, data: { type?: string; draftValue?: unknown }, updatedById: string) => {
      row = {
        id: 'cb-1',
        pageKey,
        sectionKey,
    ...
    publish: vi.fn((pageKey: string, sectionKey: string, updatedById: string) => {
      ...
      row = { ...row, publishedValue: row.draftValue, publishedAt: new Date('2026-09-05'), updatedById };
      return row;
    }),
  };
}
...
function fakeAudit() {
  ...
  return { record: vi.fn(async (input: unknown) => void records.push(input)), records };
}
...
describe('ContentService.editBlock', () => {
  it('saves a draft without publishing when publish is omitted', async () => {
    ...
    const service = new ContentService(repo, linkHealth, audit);
  ...
  it('rejects publish for a caller without content:publish', async () => {
    const repo = fakeRepo();
    const service = new ContentService(repo, fakeLinkHealth(), fakeAudit());
  ...
  it('publishes and audits when the caller holds content:publish', async () => {
    const repo = fakeRepo();
    const audit = fakeAudit();
    const service = new ContentService(repo, fakeLinkHealth(), audit);
  ...
  it('checks and tracks the link for image/link blocks, returning linkStatus', async () => {
    const repo = fakeRepo();
    const linkHealth = fakeLinkHealth();
    const service = new ContentService(repo, linkHealth, fakeAudit());
    ...
    expect(linkHealth.checkAndTrack).toHaveBeenCalledWith(
      expect.objectContaining({ url: 'https://example.org/hero.jpg', resourceType: 'content_block' }),
    );
